      ...
      <section id="afiches" className="py-20 bg-gradient-to-br from-purple-50 to-pink-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
              Descarga nuestros afiches educativos sobre los derechos humanos fundamentales
            </p>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {[
              { num: 1, title: 'Derecho a la Vida' },
              { num: 2, title: 'Libertad e Igualdad' },
              { num: 3, title: 'No Discriminación' },
              { num: 4, title: 'No Esclavitud' },
              { num: 5, title: 'No Tortura' },
              { num: 6, title: 'Reconocimiento Legal' },
              { num: 7, title: 'Igualdad ante la Ley' },
              { num: 8, title: 'Acceso a Tribunales' },
              { num: 9, title: 'No Detención Arbitraria' },
              { num: 10, title: 'Presunción de Inocencia' },
              { num: 11, title: 'Privacidad' },
              { num: 12, title: 'Libertad de Movimiento' },
              { num: 13, title: 'Asilo' }
            ].map((afiche) => (
              <Card key={afiche.num} className="overflow-hidden hover:shadow-xl transition-shadow border-0 shadow-md group">
                <div className="aspect-[3/4] bg-gray-100 overflow-hidden">
                  <img 
                    src={`/afiches/afiche-${String(afiche.num).padStart(2, '0')}.jpg`}
                    alt={`Afiche ${afiche.num}: ${afiche.title}`}
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                    loading="lazy"
                  />
                </div>
                <CardContent className="p-4">
                  <p className="text-xs text-gray-500 mb-1">Afiche {afiche.num}</p>
                  <h3 className="font-bold text-gray-900 text-sm mb-3 truncate">{afiche.title}</h3>
                  <a 
                    href={`/afiches/afiche-${String(afiche.num).padStart(2, '0')}.jpg`}
                    download={`afiche-${String(afiche.num).padStart(2, '0')}-derechos-humanos.jpg`}
